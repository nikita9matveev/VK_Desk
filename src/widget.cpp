    ...
    ui(new Ui::Widget)
{
    ...
    currentIndex = 0;  //индекс текущего окна
    ...
    l->addWidget(w_log);  //класс окна подключения
    l->addWidget(form);   //класс окна чата
...
void Widget::on_error(const QString &string)
{
    ...
    connect(errorD, SIGNAL(on_ok()), errorD, SLOT(hide()));
